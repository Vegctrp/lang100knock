...
with open(outfile,"w") as out:
    for sentence in novel_list:
        noun_concatenation_list=[]
        for word in sentence:
            if word["pos"]=="名詞":
                noun_concatenation_list.append(word["surface"])
            else:
                if len(noun_concatenation_list)>=2:
                    print(" ".join(noun_concatenation_list),file=out)
                noun_concatenation_list.clear()