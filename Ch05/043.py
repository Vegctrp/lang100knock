...
with open(outfile,"w") as out:
    for sentence in novel_list:
        for src_chunk in sentence:
            dst_chunk=sentence[src_chunk.dst]
            src_str=src_chunk.surfaces()
            if src_chunk.dst!=-1:
                dst_str=dst_chunk.surfaces()
            else:
                dst_str=""
            if src_str!="" and dst_str!="" and src_chunk.include_noun() and dst_chunk.include_verb():
