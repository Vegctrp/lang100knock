...
with open(outfile,"w") as out:
    for sentence in novel_list:
        for dst_chunk in sentence:
            if len(dst_chunk.srcs)!=0 and dst_chunk.include_verb():
                verb=dst_chunk.get_first_verb().base
                pps=[]
                chunk_surfaces=[]
                for src_chunk in dst_chunk.srcs:
                    pp=sentence[src_chunk].get_last_pparticle().base
                    chunk_surface=sentence[src_chunk].surfaces()
                    if pp!="null":
                        if not pp in pps:
                            pps.append(pp)
                        if not chunk_surface in chunk_surfaces:
                            chunk_surfaces.append(chunk_surface)
                pps.sort()
                chunk_surfaces.sort()
                if len(pps)!=0:
                    print(verb+"\t"+" ".join(pps)+"\t"+" ".join(chunk_surfaces),file=out)
